...
import azar.weather.entities.external.api.open_weather_map.lat_long_response.LatLongResponse;
...
public class WeatherManager {
    ...
    public Future<LatLongResponse> getUsingLatLong(double latitude, double longitude) {
        return Future.future(getValuePromise ->
                vertx.executeBlocking(() -> {
                    webClient.getAbs(baseUrl)
                            ...
                            .onSuccess(bufferHttpResponse -> {
                                if (bufferHttpResponse.statusCode() == 200) {
                                    ...
                                    getValuePromise.complete(jsonManager.fromJson(response, LatLongResponse.class));
                                } else {
